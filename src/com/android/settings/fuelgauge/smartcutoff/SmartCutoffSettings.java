...
public class SmartCutoffSettings extends DashboardFragment implements OnPreferenceChangeListener {
    ...
    public void onCreate(Bundle icicle) {
        ...
        int currentTemperature = getSecureValue(Settings.System.getInt(getContentResolver(),
            Settings.System.SMART_CUTOFF_TEMPERATURE, mSmartCutoffTemperatureDefaultConfig), MIN, MAX);
        mSmartCutoffTemperature.setMin(MIN);
        mSmartCutoffTemperature.setMax(MAX);
        ...
        int currentResumeLevel = getSecureValue(Settings.System.getInt(getContentResolver(),
            Settings.System.SMART_CUTOFF_RESUME_TEMPERATURE, mSmartCutoffResumeTemperatureConfig), MIN - 1, MAX);
        mSmartCutoffResumeTemperature.setMin(MIN - 1);
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        if (preference == mSmartCutoffTemperature) {
            ...
            int smartCutoffTemperature = getSecureValue((Integer) objValue, MIN, MAX);
            ...
            int smartCutoffResumeLevel = getSecureValue((Integer) objValue, MIN - 1, MAX);
    ...
    private int getSecureValue(int value, int min, int max) {
            if (value < min)
                value = min;
            else if (value > max)
                value = max;
